...
public class OnlinePistol : MonoBehaviour, IShootable
{
        ...
        foreach (Transform raypoint in raycastPoints)
        {
            if (Physics.Raycast(raypoint.position, raypoint.forward, out hit, range, shootableLayer))
            {
                if (hit.collider.CompareTag("Player") && hit.collider.gameObject != gameObject)
                {
                    ...
                    lR.enabled = true;
                    var healthScript = hit.collider.GetComponent<OnlineHealth>();
                    ...
                    healthScript.pv.RPC("TakeDamage", RpcTarget.All, damage, PhotonNetwork.LocalPlayer.ActorNumber);
                    pS.Play();
